...
const renderHeader = (
  ...
  console.log(
    `  ${statusColor('│')} ${chalk.bold('SIGNATURE:')} ${truncate(signature, 16, 16)}   ${statusText}`.padEnd(
      WIDTH + 12
    ) + `  ${statusColor('│')}`
  );
  console.log(`  ${statusColor('│')}`.padEnd(WIDTH + 5) + `  ${statusColor('│')}`);
  console.log(
    `  ${statusColor('│')} ${chalk.gray(`TRANSACTION FEE: ${displayFee} SOL`)}`.padEnd(WIDTH + 12) +
      `  ${statusColor('│')}`
  );
  console.log(`  ${statusColor('╰' + line('─') + '╯')}`);
};
...
const renderCUCost = (cuCost: CUCost | undefined) => {
  ...
  if (!cuCost || cuCost.cuConsumed === 0) {
    ...
    console.log(
      `  │  ${chalk.white('CU Consumed:')}   ${chalk.cyan(cuCost.cuConsumed.toLocaleString())} CU`.padEnd(
        WIDTH + 12
      ) + '  │'
    );
    console.log(
      `  │  ${chalk.white('Priority Fee:')}  ${priorityLabel}`.padEnd(WIDTH + 12) + '  │'
    );
    console.log(
      `  │  ${chalk.white('Fee:')}           ${chalk.yellow(cuCost.feeLamports.toLocaleString())} lamports  ·  ${chalk.yellow(feeSOLStr)} SOL  ·  ${feeUSDStr}`.padEnd(
        WIDTH + 12
      ) + '  │'
    );
  }
