...
export function renderJSON(
  ...
  try {
    ...
    const fallbackConsumed =
      analyzed?.cuProfile?.totalConsumed ?? (analyzed as any)?.computeUnits?.consumed ?? 0;
    ...
    const fallbackUtilization =
      analyzed?.cuProfile?.utilizationPercent ?? (analyzed as any)?.computeUnits?.utilization ?? 0;
    const utilization =
      analyzed?.raw?.computeUnitsConsumed != null && limit > 0
        ? (consumed / limit) * 100
        : fallbackUtilization;

    const timestamp =
      analyzed?.raw?.blockTime ??
      analyzed?.parsed?.blockTime ??
      (analyzed as any)?.blockTime ??
      null;
    const timestampISO =
    ...
    const output: RenderOutput = {
      transaction: {
        signature:
          analyzed?.raw?.signature ||
          analyzed?.parsed?.signature ||
        ...
        success:
          analyzed?.parsed?.success ??
          (analyzed?.raw ? !analyzed.raw.err : !(analyzed as any)?.error),
