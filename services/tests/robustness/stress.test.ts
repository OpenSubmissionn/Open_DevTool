...
describe('Stress test — edge cases and failure modes', () => {
  ...
  it('handles extremely nested CPI logs (15+ levels)', () => {
    const deepLogs = Array.from({ length: 15 }, (_, i) =>
      `Program PROG${i} invoke [${i + 1}]`
    )
  ...
  it('handles RPC timeout simulation (null data)', () => {
    const bundle = mockRPCBundle({ computeUnitsConsumed: null, logMessages: [], preBalances: [], postBalances: [] })
